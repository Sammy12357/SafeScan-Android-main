...
const PROCESSED_CALLBACK_CACHE_MAX = 32;

function recordProcessedCallback(url: string) {
  processedCallbackUrls.add(url);
  if (processedCallbackUrls.size > PROCESSED_CALLBACK_CACHE_MAX) {
    const oldest = processedCallbackUrls.values().next().value;
    if (oldest) processedCallbackUrls.delete(oldest);
  }
}
...
async function openPhantomSignMessage(pending: PendingFlow, session: string, walletAddress: string) {
  ...
  // iOS drops back-to-back universal-link opens silently: the second openURL
  // can fire while iOS is still mid-transition from Phantom -> SafeScan, and
  // it never reaches Phantom. Yield to the event loop + give the OS ~600ms
  // to settle so Phantom reliably foregrounds for the sign step.
...
export function useWallet() {
  const localPublicKey = useAuthStore((state) => state.walletAddress);
  const setWalletAddress = useAuthStore((state) => state.setWalletAddress);
  const isAuthenticated = useAuthStore((state) => state.isAuthenticated);
  const userId = useAuthStore((state) => state.user?.id);
  // The backend's view of wallet connection is authoritative. We mirror it
  // back to the local store so any screen using useWallet sees the same
  // answer — without this, Profile (which only read the local store) showed
  // a disconnect prompt while Airdrop (which already had the backend
  // fallback) showed the wallet connected.
  const backendWalletAddress = useAirdropStore(
    (state) => (state.status?.walletConnected ? state.status?.walletAddress ?? null : null)
  );
  const publicKey = localPublicKey ?? backendWalletAddress ?? null;
  const [isConnecting, setIsConnecting] = useState(Boolean(pendingFlow));

  // Backend-says-connected but local store is empty? Write through so the
  // next render is consistent and future authStore subscribers (logout,
  // hydrate, etc.) see the same value.
  useEffect(() => {
    if (backendWalletAddress && backendWalletAddress !== localPublicKey) {
      setWalletAddress(backendWalletAddress);
    }
  }, [backendWalletAddress, localPublicKey, setWalletAddress]);
  ...
  const connect = useCallback(async () => {
    ...
    if (pendingFlow || isConnecting) {
      throw new Error("Another Phantom connection is already in progress.");
    }
