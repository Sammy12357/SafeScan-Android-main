...
import { api, clearApiTokens, setApiTokens, setTokenPersister, type User } from "@/services/api";
import { roleForEmail } from "@/constants/config";

const AUTH_TOKEN_KEY = "auth_token";
const REFRESH_TOKEN_KEY = "refresh_token";

function persistTokensToSecureStore(tokens: { accessToken: string; refreshToken: string }) {
  void SecureStore.setItemAsync(AUTH_TOKEN_KEY, tokens.accessToken).catch(() => {});
  void SecureStore.setItemAsync(REFRESH_TOKEN_KEY, tokens.refreshToken).catch(() => {});
}

// Wires silent refreshes inside the API client back to SecureStore so a stale
// access token isn't rehydrated on cold start.
setTokenPersister(persistTokensToSecureStore);
