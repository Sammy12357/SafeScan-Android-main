...
export function buildLocalOnlyAnalyzeResult(payload: string): AnalyzeResult {
  const trimmed = payload.trim();
  const local = computeUrlRisk(trimmed);
  const score = clampScore(local.score);
  const verdict = verdictFromScore(score);
  const analyzedAt = new Date().toISOString();
  const offlineNotice = signal(
    "Offline analysis",
    "medium",
    "Server unreachable — heuristic verdict only.",
    "SafeScan ran on-device rules only because the network was offline. Re-scan once you're back online to confirm with the full analyzer.",
    false
  );
  const decodedUrl = firstUrl(trimmed) || trimmed;
  const url = decodedUrl || trimmed || "Offline scan";
  ...
  return {
    ...
    verdictText:
      verdict === "danger"
        ? "On-device checks flagged high-risk patterns. Do not open until SafeScan can verify online."
        : verdict === "warn"
          ? "On-device checks found suspicious patterns. Re-scan online to confirm."
          : "On-device checks found no obvious red flags, but SafeScan could not reach the server.",
    overallRisk: legacyRiskFromVerdict(verdict),
    signals: dedupeSignals([offlineNotice, ...local.signals]),
    analyzedAt,
    scannedAt: analyzedAt,
    payloadType: decodedUrl ? "url" : "text",
    source: "local-only",
    ruleScore: score,
